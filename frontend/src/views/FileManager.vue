    ...
    <div v-else class="fm-grid">
      <div v-for="f in list" :key="f.id" class="file-card">
        ...
        <div class="file-actions">
          <a :href="f.download_url" class="action-btn" title="下载" download>↓</a>
          ...
          <button @click="deleteFile(f)" class="action-btn del" title="删除">×</button>
        </div>
...
function startUpload(e) {
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      item.progress = 100
      try {
        files.value.unshift(JSON.parse(xhr.responseText))
      } catch (e) {
        fetchFiles()
      }
...
async function toggleFav(f) {
  try {
    ...
    if (r.ok) f.is_favorite = (await r.json()).is_favorite
  } catch (e) {
    console.error(e)
  }
}
